    ...
    switch (regAddr) {
      case 0x1: {
        ...
        if (0x80 & regVal) {
          ...
          ESP_LOGCONFIG(TAG, "0 -> Operating mode as selected with Mode bits in RegOpMode is automatically reached "
                             "with the Sequencer\n");
        }
      ...
      case 0xb: {
        ESP_LOGCONFIG(TAG, "Improved AFC routine for signals with modulation index lower than 2.  Refer to section "
                           "3.4.16 for details\nAfcLowBetaOn : ");
        if (0x20 & regVal) {
      ...
      case 0xd: {
        ...
        if (val == 0b00) {
          ...
          ESP_LOGCONFIG(TAG,
                        "01 -> chip stays in Rx mode until PayloadReady or Timeout interrupt occurs.  It then goes to "
                        "the mode defined by Mode. Listen mode stops and must be disabled (see section 4.3)\n");
        } else if (val == 0b10) {
          ESP_LOGCONFIG(TAG, "10 -> chip stays in Rx mode until PayloadReady or Timeout occurs.  Listen mode then "
...
void RemoteTransmitterComponent::set_mode(uint32_t mode) { rf12_control(mode); }
...
void RemoteTransmitterComponent::send_internal(uint32_t send_times, uint32_t send_wait) {
  ...
  ESP_LOGD(TAG, "Sending rfm12 remote code... %d %d %d %s", send_wait, send_times, this->temp_.get_data().size(),
           s.str().c_str());
  vspi.setFrequency(80 * 1000000);
