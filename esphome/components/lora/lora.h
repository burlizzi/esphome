...
protected:
  ...
  void on_binary_sensor_update(binary_sensor::BinarySensor *obj, bool state) override{send(obj->get_object_id_hash(),state );}
#endif
#ifdef USE_COVER
  void on_cover_update(cover::Cover *obj) override{ESP_LOGE("lora","not implemented");}
#endif
#ifdef USE_FAN
  void on_fan_update(fan::FanState *obj) override{ESP_LOGE("lora","not implemented");}
#endif
#ifdef USE_LIGHT
  void on_light_update(light::LightState *obj) override{ESP_LOGE("lora","not implemented");}
#endif
#ifdef USE_SENSOR
  ...
  void on_text_sensor_update(text_sensor::TextSensor *obj, const std::string &state) override {send(obj->get_object_id_hash(),state );}
#endif
#ifdef USE_CLIMATE
  void on_climate_update(climate::Climate *obj) override{ESP_LOGE("lora","not implemented");}
#endif
#ifdef USE_NUMBER
  void on_number_update(number::Number *obj, float state) override {/*send(obj->get_object_id_hash(),state );*/}
#endif
#ifdef USE_SELECT
  void on_select_update(select::Select *obj, const std::string &state) override{send(obj->get_object_id_hash(),state );}
#endif            
          template <typename T>
          inline void send(uint32_t hash,const T& data)
          {
            send(hash,std::string((char*)&data,sizeof(data)));
          }
          
          inline void send(uint32_t hash,const std::string& data)
          {
            send(std::string((char*)&hash,sizeof(hash))+data);
          }
        };
    }
}
